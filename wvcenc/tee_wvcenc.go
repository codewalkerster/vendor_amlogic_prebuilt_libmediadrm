...
func init() {
    ...
    android.RegisterModuleType("liboemcrypto_atsc_go_defaults", liboemcrypto_atsc_DefaultsFactory)
}
...
func liboemcrypto_atsc_DefaultsFactory() (android.Module) {
    module := cc.DefaultsFactory()
    android.AddLoadHook(module, func(ctx android.LoadHookContext) {
        var props common.Props
        info := common.Information{
            FileString: "liboemcrypto_atsc.so",
            SrcPath: "vendor/amlogic/common/widevine",
            System: false,
            MultiArch: true,
        }
        ...
        common.SetProps(ctx, &props, info)
        p := &props
        fmt.Println("liboemcrypto_atsc p.Arch.Arm.Srcs", p.Arch.Arm.Srcs)
        fmt.Println("liboemcrypto_atsc p.Arch.Arm64.Srcs", p.Arch.Arm64.Srcs)
        ctx.AppendProperties(p)
    })
    return module
}
